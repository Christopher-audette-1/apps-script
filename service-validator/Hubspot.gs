/**
 * @file Hubspot.gs
 * @description Connection test for the Hubspot API.
 */
...
function testHubspotConnection() {
  ...
  if (!apiKey) {
    return {
      service: serviceName,
      status: 'FAILURE',
      message: `[${serviceName}] API key not found. Please ensure HUBSPOT_API_KEY is configured in Google Secret Manager.`,
    };
  }
